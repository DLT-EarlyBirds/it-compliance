...
Together they build what we call a regulation graph that specifies the law regulations, their rules (sub-regulations) and the claims that need to be proven by a financial service provider to be compliant to the laws.
...
# Issuing flows via ssh:
You can ssh into each of the dockerized nodes to run the flows of the CorDapp manually via the commandline. For this simply type the following command with password `test`: 

```console
 $ ssh -p <node-ssh-port> localhost -l user1
```

The node ports are the following:

Auditor: `2223`

Supervisory Authority: `2224`

Brain Finance: `2225`

Capitals Holding: `2226`


In the following we are listing all runnable commands to instantiate flows. Some of them require inputs generated by previous flows (e.g. the linearId of another state). To fetch this data please run the vault queries.
## Regulations
