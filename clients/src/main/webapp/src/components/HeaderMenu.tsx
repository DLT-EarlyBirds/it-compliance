    ...
    return (
        <>
            <div className="flex justify-between h-full">
                <Link to="/">
                    {currentNode === NodeEnum.BRAIN_FINANCE && <img className="self-center m-2 h-14" src={BrainFinance} />}
                    {currentNode === NodeEnum.CAPITALS_HOLDING && <img className="self-center m-2 h-14" src={CapitalsHolding} />}
                    {currentNode === NodeEnum.SUPERVISORY_AUTHORITY && <img className="self-center m-2 h-14" src={SupervisoryAuthority} />}
                    {currentNode === NodeEnum.AUDITOR && <img className="self-center m-2 h-14" src={Auditor} />}
                </Link>
